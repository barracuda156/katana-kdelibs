...
static QFont nearestExistingFont(const QFont &font)
{
    ...
    if (!dbase.isScalable(family, style)) {
        const QList<int> sizes = dbase.pointSizes(family, style);
        if (!sizes.contains(size)) {
            // Find nearest available size.
            int mindiff = 1000;
            int refsize = size;
            foreach (const int lsize, sizes) {
...
void KFontRequester::KFontRequesterPrivate::displaySampleText()
{
    ...
    if (m_sampleText.isEmpty()) {
        ...
        m_sampleLabel->setText(QString::fromLatin1( "%1 %2").arg(family).arg(KGlobal::locale()->formatNumber(size, (size == floor(size)) ? 0 : 1)));
    } else {
